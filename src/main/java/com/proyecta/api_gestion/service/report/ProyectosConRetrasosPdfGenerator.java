    ...
    private static final class PdfCanvas {
        ...
        private List<DeliverableRow> buildDeliverableRows(List<EntregablePendienteDTO> entregables) {
            if (entregables == null || entregables.isEmpty()) {
                return List.of(
                        new DeliverableRow("1", "", "[DD/MM/AAAA]"),
                        new DeliverableRow("2", "", "[DD/MM/AAAA]"),
                        new DeliverableRow("3", "", "[DD/MM/AAAA]")
                );
            }
            ...
            for (EntregablePendienteDTO entregable : entregables) {
                rows.add(new DeliverableRow(
                        ...
                        entregable.fechaEntrega() != null ? formatDate(entregable.fechaEntrega()) : "[DD/MM/AAAA]"
                ));
            }
            return rows;
        }
        ...
        private float drawTableHeader(float topY, String[] headers, float[] widths, float[] sizes) throws IOException {
            float maxHeight = 0f;
        ...
        private void drawProjectRow(ProyectoReporteResumenDTO row, float[] widths, float rowHeight) throws IOException {
            ...
            for (int i = 0; i < widths.length; i++) {
                ...
                float fontSize = i == 2 ? 9.6f : 9.2f;
                drawText(values[i], fonts.regular(), fontSize, x + 4f, y - 11f, COLOR_TEXT);
                x += colWidth;
            }
        }
        ...
        private void drawDeliverableRow(DeliverableRow row, float[] widths, float rowHeight) throws IOException {
            float x = LEFT;
            float y = cursorY;
            String[] values = {row.numero(), row.nombre(), row.fecha()};
            drawFilledRect(x, y - rowHeight, CONTENT_WIDTH, rowHeight, Color.WHITE);
            drawRect(x, y - rowHeight, CONTENT_WIDTH, rowHeight, COLOR_BORDER, 0.6f);
            for (int i = 0; i < widths.length; i++) {
                float colWidth = CONTENT_WIDTH * widths[i];
