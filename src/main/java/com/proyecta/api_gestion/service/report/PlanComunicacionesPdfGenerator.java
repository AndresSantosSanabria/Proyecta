...
public final class PlanComunicacionesPdfGenerator {
    ...
    private record RowData(String codigo, String nombre, String director, String dependencia, boolean tienePlan) {
    }
    ...
    private static final class PdfCanvas {
        ...
        void render(List<Proyecto> proyectos, LocalDate corte, DetailMode mode) throws IOException {
            ...
            if (rows.isEmpty()) {
                drawEmptyState("No hay proyectos No PETI registrados.");
            } else {
        ...
        private void renderSummary(int countYes, int countNo) throws IOException {
            drawLabelValue("Conteo de proyectos con plan de comunicaciones:", String.valueOf(countYes), 560f, false);
            drawLabelValue("Conteo de proyectos sin plan de comunicaciones:", String.valueOf(countNo), 538f, false);
            cursorY = 500f;
        }
        ...
        private void drawRow(RowData row, float rowHeight) throws IOException {
            float x = LEFT;
            float y = cursorY;
            float[] widths = widths();
            String[] values = {
        ...
        private void drawTableHeader(float topY) throws IOException {
            ...
            float[] sizes = {8.8f, 8.8f, 8.6f, 8.8f, 8.4f};
            float[] widths = widths();
            float maxHeight = 0f;
        ...
        private float rowHeight(RowData row) throws IOException {
            float maxLines = 1f;
            float[] widths = widths();
            String[] values = {
                    row.codigo(),
