...
public final class FuragPdfGenerator {
    ...
    private static boolean furagCompleto(Proyecto proyecto) {
        Furag furag = proyecto != null ? proyecto.getFurag() : null;
        return furag != null
    ...
    private static List<FuragRow> buildRows(List<Proyecto> proyectosDependencia) {
    ...
    private record FuragRow(String codigo, String nombre, String director, String dependencia, boolean responde) {
    }
    ...
    private static final class PdfCanvas {
        ...
        void render(Proyecto proyecto, List<Proyecto> proyectosDependencia, LocalDate corte, DetailMode mode) throws IOException {
            ...
            if (rows.isEmpty()) {
                drawEmptyState("No hay proyectos registrados para la dependencia seleccionada.");
            } else {
                for (FuragRow row : rows) {
        ...
        private void renderSummary(int countYes, int countNo) throws IOException {
            drawLabelValue("Conteo de proyectos que responden preguntas FURAG:", String.valueOf(countYes), 560f, false);
            drawLabelValue("Conteo de proyectos que NO responden preguntas FURAG:", String.valueOf(countNo), 538f, false);
            cursorY = 500f;
        }
        ...
        private void drawRow(FuragRow row, float rowHeight) throws IOException {
            float x = LEFT;
            float y = cursorY;
            float[] widths = widths();
            String[] values = {
        ...
        private void drawTableHeader(float topY) throws IOException {
            ...
            for (int i = 0; i < headers.length; i++) {
                ...
                maxHeight = Math.max(maxHeight, Math.max(28f, wrapped.size() * 10.5f + 8f));
            }
